...
<div class="pd-20">
	<form action="" method="post" class="form form-horizontal" id="admin_product" >
		<div class="row cl">
			<label class="form-label col-2"><span class="c-red">*</span>分类名：</label>
			<div class="formControls col-4">
				<input type="text" class="input-text" value="${requestScope.product_type.name}" placeholder=""  name="name">
				<c:choose>
					<c:when test="${requestScope.add_or_edit==false}">
						<input type="hidden" class="input-text" value="${requestScope.product_type.id}" placeholder=""  name="id">
					</c:when>
				</c:choose>
			</div>
		</div>
...
function  Button(){
    if(${requestScope.add_or_edit==true}) {
        $.post("<%=basePath%>admin/index/product_type/add/post",
        ...
        $.post("<%=basePath%>admin/index/product_type/edit/post",
            $("#admin_product").serialize(),
            function (data) {
                if (data == "1") {
