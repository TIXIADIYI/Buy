...
<div class="pd-20">
	<form action="/" method="post" class="form form-horizontal"  id="admin_">
		<div class="row cl">
			...
			<div class="formControls col-4"> ${sessionScope.admin.id} </div>
			<div class="col-4"><input name="id" value="${sessionScope.admin.id}" type="hidden"> </div>
		</div>
		<div class="row cl">
			<label class="form-label col-4"><span class="c-red">*</span>账号名：</label>
			<div class="formControls col-4">
				<input name="name" value="${sessionScope.admin.name}" type="text" class="input-text" autocomplete="off"    >
			</div>
			<div class="col-4"> </div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-4">
				<input name="pass" type="password" class="input-text" autocomplete="off"    >
			</div>
			<div class="col-4"> </div>
		</div>
		<div class="row cl">
...
<script>
    function  Button(){
            $.post("<%=basePath%>admin/index/pass/post",
                $("#admin_").serialize(),
                function (data) {
                    if (data == "1") {
                        layer.msg('修改成功!', {icon: 6, time: 1000});
                    } else if (data == "2") {
                        layer.msg('修改失败，填写不完整', {icon: 5, time: 1000});
                    }else if(data=="3"){
                        layer.msg('修改失败，两次密码不一致', {icon: 5, time: 1000});
					}else{
