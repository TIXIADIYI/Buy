...
<body class="pos-r">
<div >
	<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 产品管理 <span class="c-gray en">&gt;</span> 产品评论 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
	<div class="pd-10">
		<div class="cl pd-5 bg-1 bk-gray "> <span class="l">
			<a href="#" id="datadel" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
			<a class="btn btn-primary radius" onclick="product_add('添加产品','<%=basePath%>admin/index/product_comment/add')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加评论</a>
		</span>
			<span class="r">共有数据：<strong>${fn:length(requestScope.product_comment)}</strong> 条</span> </div>
		<div class="mt-20">
			<table class="table table-border table-bordered table-bg table-hover table-sort">
                ...
                <c:forEach items="${requestScope.product_comment}" var="list">
					<tr class="text-c va-m">
						...
						<td >${list.id}</td>
						<td >${list.product_id.name}</td>
						<td><img width="60" class="product-thumb" src="${list.product_id.image}"></td>
						<td > ${list.user_id.user}(${list.user_id.name})</td>
						<td >${list.comment}</td>
						<td ><fmt:formatDate type="date" value="${list.time}" dateStyle="default" /></td>
						<td >
							<c:choose>
								<c:when test="${list.praise==true}">
									<span class="c-green">好评</span>
								</c:when>
								<c:otherwise>
									<span class="c-red">差评</span>
								</c:otherwise>
							</c:choose>
						</td>
						...
						<td class="td-manage">
							<a style="text-decoration:none" class="ml-5" onClick="product_edit('产品编辑','<%=basePath%>admin/index/product_comment/edit?id=${list.id}','')" href="javascript:;" title="编辑">
...
function product_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
        $.get("<%=basePath%>admin/index/product_comment/del?id="+id+" ",
            function (data) {
                if(data=="1"){
...
$("#datadel").click(function (){
    layer.confirm('确认要删除吗？',function(index){
       ...
       for(var i=0;i<s.length;i++){
           if(s[i].checked){
               $.get("<%=basePath%>admin/index/product_comment/del?id="+s[i].value+" ",
                   function (data) {
                   });
		   }
	   }
