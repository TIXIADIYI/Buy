...
<body class="pos-r">
<div >
	<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 管理员功能 <span class="c-gray en">&gt;</span> 管理员列表 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
	<div class="pd-10">
		<div class="cl pd-5 bg-1 bk-gray "> <span class="l">
			<a href="#" id="datadel" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
			<a class="btn btn-primary radius" onclick="product_add('添加分类','<%=basePath%>admin/index/add')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加管理员账号</a>
		</span>
			<span class="r">共有数据：<strong>${fn:length(requestScope.admin)}</strong> 条</span> </div>
		<div class="mt-20">
			<table class="table table-border table-bordered table-bg table-hover table-sort">
				...
				<c:forEach items="${requestScope.admin}" var="list">
					<tr class="text-c va-m">
						<td>
							<c:choose>
							<c:when test="${list.id==sessionScope.admin.id}">

							</c:when>
								<c:otherwise>
									<input name="checkbox" type="checkbox" value="${list.id}" >
								</c:otherwise>
							</c:choose>
						</td>
						<td >${list.id}</td>
						<td class="text-l"> ${list.name}</td>
						<td class="text-l"> ${list.pass}</td>
						<td class="td-manage">
							<c:choose>
								<c:when test="${list.id==sessionScope.admin.id}">
									正在使用该账号登录
								...
								<c:otherwise>
									<a style="text-decoration:none" class="ml-5" onClick="product_edit('分类编辑','<%=basePath%>admin/index/edit?id=${list.id}','')" href="javascript:;" title="编辑">
...
<script type="text/javascript">
    ...
    function product_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.get("<%=basePath%>admin/index/del?id="+id+" ",
                function (data) {
                    if(data=="1"){
    ...
    $("#datadel").click(function (){
            layer.confirm('确认要删除吗？',function(index){
                ...
                for(var i=0;i<s.length;i++){
                    if(s[i].checked){
                        $.get("<%=basePath%>admin/index/del?id="+s[i].value+" ",
                            function (data) {
                            });
                    }
                }
